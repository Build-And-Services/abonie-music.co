...
<body class="relative min-h-screen backdrop-blur-md"
      @if($presave->photo != null)
          style="background-image: url({{ $presave->photo }});    background-repeat: no-repeat;
    ...
    <div class="links grid grid-cols-12 gap-4">
        ...
        @else
            <div class="col-span-12 flex flex-col gap-3 mt-2">
                @foreach($presave->links as $link)
                    <div
                        class="group relative col-span-12 flex h-[3.6rem] cursor-pointer items-center justify-between rounded-lg border border-black bg-white px-4 transition-all duration-300 hover:scale-105"
