@props(['title', 'total', 'active', 'inactive'])
<div class="card rounded-lg shadow-lg dark:border-zinc-600 dark:bg-zinc-800">
    <div class="card-body px-6 py-4">
            ...
            <div class="flex flex-col">
                ...
                <div class="flex w-full flex-col">
                    <h4 class="text-21 font-2xl my-4 font-bold text-gray-800 dark:text-gray-100">
                        <span class="counter-value">Total {{$title}} : {{ $total }}</span>
        ...
        <div class="row gap-2">
            <span class="text-13 font-medium text-gray-700 dark:text-zinc-100">Total {{ $title }} Active : </span>
            <span
                class="rounded bg-green-500/40 px-1 py-[1px] text-[10px] font-medium text-green-500 dark:bg-green-500/30">
                {{$active}}</span>
            <span class="text-13 font-medium text-gray-700 dark:text-zinc-100">Total {{ $title }} Inactive : </span>
            <span
                class="rounded bg-red-500/40 px-1 py-[1px] text-[10px] font-medium text-red-500 dark:bg-red-500/30">
                {{$inactive}}</span>
        </div>
    </div>
</div>
