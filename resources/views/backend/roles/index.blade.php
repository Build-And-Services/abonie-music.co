        ...
        <div class="col-span-12">
            <div class="card dark:bg-zinc-800 dark:border-zinc-600">
                <div class="card-body flex items-center justify-between border-b border-gray-100 dark:border-zinc-600">
                    ...
                    @can('role.create')                        
                        <button class="text-white btn bg-violet-500 border-violet-500 hover:bg-violet-600 hover:border-violet-600 focus:bg-violet-600 focus:border-violet-600 focus:ring focus:ring-violet-500/30 active:bg-violet-600 active:border-violet-600"  data-tw-target="#modal-id_form" data-tw-toggle="modal">Create Role</button>
                    @endcan
                    
                    <div class="relative z-50 modal @if ($errors->any()) p @else hidden    @endif" id="modal-id_form" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                            ...
                            <div class="p-4 mx-auto animate-translate sm:max-w-lg">
                                <div class="relative overflow-hidden text-left transition-all transform bg-white rounded-lg shadow-xl dark:bg-zinc-600">
                                    <div class="bg-white dark:bg-zinc-700">
                                        ...
                                        <div class="p-5">
                                            <h3 class="mb-4 text-xl font-medium text-gray-700 dark:text-gray-100">Create Role</h3>
                                            <form class="space-y-4" action="{{ route('roles.store') }}" method="post">
                                                @csrf
                                                <div>
                                                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-100 ltr:text-left rtl:text-right">New Name</label>
                                                    <input type="text" name="name" value="{{ old("name") }}" id="name" class="bg-gray-800/5 border border-gray-100 text-gray-900 dark:text-gray-100 text-sm rounded focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-zinc-700/50 dark:border-zinc-600 dark:placeholder-gray-400 dark:placeholder:text-zinc-100/60 focus:ring-0" placeholder="Cashier" required="">
                                                    @error('name')
                    ...
                    <x-backend.table :datas="$roles" :columns="['name', 'guard name']" name="roles">
                        @foreach ($roles as $cell)
                            <tr>
                                ...
                                <x-backend.column-table>
                                    <x-backend.btn.action-detail name="roles" :id="$cell->id" :modal="true">
                                        detail
                                    </x-backend.btn.action-detail>
                              
                                    <x-backend.btn.action-edit name="roles" :id="$cell->id" :modal="true" target="#edit-modal-{{ $cell->id }}">
                                        edit
